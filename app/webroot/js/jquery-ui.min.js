...
$(document).ready(function(){
  ...
  var slcrr = 0;
    var timeoutID;
    var sltotal = $('.slider-item').length;
    var delaytimer = 0;
    ...
    function showItem(id){
        ...
        if($('.slcrr').length){
            $('.slcrr').fadeOut(800,function(){
                $(this).find('.sub').css('top',385)
            });
        }
        
        $('.slider-item').removeClass('slcrr').eq(id).delay(delaytimer).fadeIn(1200,function(){
             $('.at-nav li').removeClass();
             $('.at-nav li').eq(id).addClass('active');

            if($(this).find('.sub').hasClass('subtop')){
                $(this).find('.sub').each(function(index, val) {
                   $(this).delay(index*300).css('top',-385).animate({'top':0},800,'easeOutCirc')
                })
           }else{
             $(this).find('.sub').each(function(index, val) {
                   $(this).delay(index*300).animate({'top':0},800,'easeOutCirc')
                })
           }
             
            $('.at-slider').removeClass('isPlay');
            $('.control-wrap span').html((slcrr +1 )+' of ' + sltotal)
...
if($("#allinone_carousel_charming ul.allinone_carousel_list li").length) {
...
//$('.main-nav li').hover(function(){
 //   if($(this).children('.sub-menu').length){
  //    $(this).children('.sub-menu').stop(true, true).slideDown('normal');
   // }
    //},function(){
...
// An hien menu
  ...
  /*Search item*/ 
     ...
     $('.search-wrap').css({'width':'24px'},{'border':'none'}).mouseenter(function(){
    $(this).animate({'width':'135px'},300).addClass('border');
    // $('.searchbutton').css({'top':'0'});
  }).mouseleave(function(){
    $('.search-wrap').removeClass('border');
    if($("input,textarea").is(":focus")){
    //input and text area has focus
    }
    else{
      setTimeout(function(){$('.search-wrap').animate({'width':'24px'},100)},500)
      // $('.searchbutton').css({'top':'-15px'});
    }
  })
  $('.search-wrap .searchinput').focusout(function() {
    setTimeout(function(){$('.search-wrap').animate({'width':'24px'},100)},500)
    // $('.searchbutton').css({'top':'-15px'});
  });

      $('.page-title').append($('.name-title')).append($('.sitemap-heading'));
      $('.full-page').parents('.content').addClass('full');


    //   if($('.desc-news').length > 0) {
    //   $( ".desc-news h4 a" ).each(function() {
    //     var A = $(this).html().toString();
    //     var B = (A.length > 60) ? A.substring(0, 55) + "..." : A;
    //     if (A.indexOf("&") != -1) {
    //       B = (A.length > 60) ? A.substring(0, 56) + "..." : A
    //     }
    //     $(this).html(B);
    //   });
    // };

    $('.Prj-d').parent().find('.pages').hide();

    $('.show-b').live( "click", function() {
      $(this).addClass('hide-b');
       $(this).removeClass('show-b');
      $('.small-img-prj').animate({ "right": "-78px" }, 900);
    })

$('.hide-b').live( "click", function() {
      $(this).addClass('show-b');
       $(this).removeClass('hide-b');
      $('.small-img-prj').animate({ "right": "0px" }, 900);
    })
    ...
    //$('.showapplyfrm').colorbox({width:'750px', height:'490px', iframe:true, overlayClose:false});
})
